...
# ── Training Augmentations (on-the-fly, train only — not at inference) ─
# Variants of each recorded frame so the CNN sees more lighting / blur /
# noise / geometry without driving extra laps. FLIP also negates steering.
AUGMENTATIONS = [
    'BRIGHTNESS',   # day ↔ shadow (brightness + contrast)
    'BLUR',         # soft / defocus blur
    'MOTION_BLUR',  # motion blur
    'NOISE',        # sensor noise
    'COLOR',        # hue / saturation / RGB shifts
    'CROP',         # mild random crop/pad
    'WARP',         # small translate / scale / rotate
    'FLIP',         # horizontal flip + negate steering label
]
AUG_PROB = 0.5              # probability each listed transform is applied
AUG_FLIP_PROB = 0.5         # probability of horizontal flip when FLIP enabled
AUG_BRIGHTNESS_RANGE = 0.2  # brightness/contrast limit ±0.2
AUG_BLUR_RANGE = (0.0, 3.0) # gaussian blur sigma range
AUG_MOTION_BLUR_LIMIT = 7   # motion blur kernel size (odd int)
AUG_NOISE_STD = (0.02, 0.08)
AUG_HUE_SHIFT = 15
AUG_SAT_SHIFT = 25
AUG_VAL_SHIFT = 15
AUG_RGB_SHIFT = 15
AUG_CROP_PERCENT = 0.08     # ±8% crop/pad
AUG_WARP_TRANSLATE = 0.05   # ±5% translation
AUG_WARP_SCALE = 0.05       # ±5% scale
AUG_WARP_ROTATE = 5.0       # ±5 degrees
...
VESC_MAX_SPEED_PERCENT = .2     # 50% of max duty; raise carefully
VESC_SERIAL_PORT = "/dev/serial/by-id/usb-STMicroelectronics_ChibiOS_RT_Virtual_COM_Port_304-if00"  # Change to /dev/ttyACM1 if the camera is using ACM0
...
# Steering center is set in VESC Tool (not donkey calibrate). See TA instructions:
#   App Settings → General → Enable Servo Output (PPM app must be Off)
...
AUTO_CREATE_NEW_TUB = True  # needed so line_follower schema doesn't clash with manage.py tubs
#
...
#
# Lane follow (lane_follow.py / lane_follower_part.py)
# Color HSV from OAK-D calibration (dashed line is TEAL on camera, not yellow)
#
...
CV_CONTROLLER_CONDITION = None

LANE_SIDE = "left"  # "left" | "right"
...
# "threshold" / "canny" / "both" = grayscale COM fallbacks
LANE_DETECT_MODE = "color"

# Cleaned from calib_20260716_215532 (dropped dark false "white" clicks V~90)
# Dashed divider on OAK: H~87-95 (teal/cyan)
YELLOW_THRESHOLD_LOW = (80, 50, 100)
YELLOW_THRESHOLD_HIGH = (105, 220, 210)
YELLOW2_THRESHOLD_LOW = (0, 0, 0)
YELLOW2_THRESHOLD_HIGH = (0, 0, 0)
# Solid edge: low sat, high value (auto-excludes yellow mask in code)
WHITE_THRESHOLD_LOW = (50, 0, 150)
WHITE_THRESHOLD_HIGH = (110, 40, 220)

LANE_BLUR = 5
LANE_BINARY_THRESH = 150          # only used if LANE_DETECT_MODE != color
LANE_CANNY_LOW = 60
LANE_CANNY_HIGH = 160
LANE_SIDE_ROI_FRAC = 0.70

TARGET_PIXEL = None
TARGET_THRESHOLD = 10
CONFIDENCE_THRESHOLD = 0.002
MIN_LANE_WIDTH_PX = 20
MAX_LANE_WIDTH_PX = 140
NOMINAL_HALF_WIDTH_PX = 40
CENTER_SMOOTHING = 0.3

COLOR_THRESHOLD_LOW = YELLOW_THRESHOLD_LOW
COLOR_THRESHOLD_HIGH = YELLOW_THRESHOLD_HIGH

# With VESC_MAX_SPEED_PERCENT=0.2, duty = throttle * 0.2.
# 0.15 -> 3% duty is often too weak to roll; use a bit more for lane follow.
THROTTLE_MAX = 0.45
THROTTLE_MIN = 0.25
...
# ==============================================================================
# SUNGSAN: ISOLATED SINGLE-LINE FOLLOWING EXPERIMENT
# This file is intentionally separate from the team's shared myconfig.py.
# ==============================================================================
...
# The OAK-D part returns OpenCV BGR frames. Decode those frames correctly and
# track the physical yellow tape rather than its channel-swapped cyan display.
...
# ==============================================================================
# SUNGSAN: ISOLATED TWO-LANE FOLLOWING EXPERIMENT
# Final assignments below override the earlier shared and single-line blocks
# while preserving the vehicle-tested OAK-D, VESC, and steering calibration.
# ==============================================================================

WEB_CONTROL_PORT = 8893
WEB_INIT_MODE = "user"
DATA_PATH = "/home/pi/mycar/data_lane_following_sungsan"
AUTO_CREATE_NEW_TUB = True

CV_CONTROLLER_MODULE = "lane_following_controller_sungsan"
CV_CONTROLLER_CLASS = "LaneFollower"
CV_CONTROLLER_INPUTS = ['cam/image_array']
CV_CONTROLLER_OUTPUTS = ['pilot/steering', 'pilot/throttle', 'cv/image_array']
CV_CONTROLLER_CONDITION = "run_pilot"

# Default only; lane_following_drive_sungsan.py --lane=left/right overrides it.
LANE_SIDE = "left"

# The OAK-D part returns OpenCV BGR frames. These thresholds use the same
# physical-yellow calibration that was vehicle-tested for line following.
CV_INPUT_COLOR_ORDER = "BGR"
...
# White tape is low-saturation and bright. The channel-spread condition and
# connected-component width limit reject blue tape, cones, walls, and pavement.
LANE_WHITE_THRESHOLD_LOW = (0, 0, 135)
LANE_WHITE_THRESHOLD_HIGH = (180, 70, 255)
LANE_WHITE_MAX_CHANNEL_SPREAD = 70

# Four perspective bands, expressed at the 160x120 reference resolution.
LANE_SCAN_Y = 68
LANE_SCAN_HEIGHT = 8
LANE_SCAN_COUNT = 4
LANE_SCAN_STEP = 12
...
# Track geometry. The controller scales these values to the live OAK-D frame.
...
# Conservative first-test speed; increase only after both lanes pass raised-
# wheel local_angle testing and low-speed physical local-mode testing.
THROTTLE_MIN = 0.25
THROTTLE_INITIAL = THROTTLE_MIN
THROTTLE_MAX = 0.35
THROTTLE_STEP = 0.02
