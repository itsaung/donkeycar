    ...
    def run(self, cam_img):
        ...
        if confidence >= self.confidence_threshold:
            ...
            if self.no_line_count >= self.restart_boost_after_lost:
                self.start_boost_remaining = self.start_boost_frames
            # Never keep driving with a steering command from an older frame.
            # A stale command was causing the car to turn hard right even
            # while the overlay reported LINE X:0 / CONF:0.
            self.throttle = 0.0
            self.steering = 0.0
